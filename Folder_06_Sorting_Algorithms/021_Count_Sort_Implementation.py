def countsort(A):
    ...
    carray = [0] * (
        maxsize + 1
    )  # Tạo một mảng carray có kích thước maxsize + 1 để đếm số lần xuất hiện của mỗi giá trị

    for i in range(n):
        carray[A[i]] = (
            carray[A[i]] + 1
        )  # Đếm số lần xuất hiện của mỗi giá trị trong danh sách
