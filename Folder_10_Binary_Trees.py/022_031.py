...
class _Node:  # Định nghĩa lớp _Node để đại diện cho một nút trong cây nhị phân
    __slots__ = (
        ...
        "_left",
        "_right",
    )  # để tối ưu hóa việc lưu trữ dữ liệu bằng cách chỉ định các thuộc tính được phép của đối tượng. Trong trường hợp này, các thuộc tính được phép bao gồm _element, _left, và _right.

    def __init__(
        self, element, left=None, right=None
    ):  # Hàm khởi tạo của lớp _Node với các tham số element, left, và right. Tham số left và right có giá trị mặc định là None.
...
class BinaryTree:  # Định nghĩa một lớp  BinaryTree.
    ...
    def maketree(
        self, e, left, right
    ):  # Định nghĩa một phương thức maketree cho lớp BinaryTree với ba tham số: e, left, và right.
        self._root = _Node(
            e, left._root, right._root
        )  # Tạo một đối tượng _Node mới với các tham số e, left._root, và right._root, sau đó gán đối tượng này cho thuộc tính _root của đối tượng BinaryTree hiện tại.

    def inorder(
        self, troot
    ):  # hàm duyệt cây theo inoder( duyệt từ cây con bên trái cùng
        if troot:  # kiểm tra nếu troot không phải là None
            self.inorder(
                troot._left
            )  # duyệt từng nút con bên trái của một nút theo thứ tự trước và thực hiện đệ quy cho đến khi không còn nút con nào,trước khi tiếp tục duyệt nút con bên phải.)
            print(troot._element, end=" ")  # in ra giá trị hiện tại
            self.inorder(
                troot._right
            )  # cho đến khi không còn nút con nào, trước khi tiếp tục duyệt nút con bên phải
    ...
    def preorder(
        self, troot
    ):  # hàm duyệt cây theo preoder (duyệt từng nút con bên trái của một nút theo thứ tự trước và thực hiện đệ quy cho đến khi không còn nút con nào, trước khi tiếp tục duyệt nút con bên phải.)
        if troot:  # kiểm tra nếu troot không phải là None
            print(troot._element, end=" ")  # in ra giá trị đang duyệt tới hiện tại
            self.preorder(
                troot._left
            )  # duyệt từng nút con bên trái của một nút theo thứ tự trước và thực hiện đệ quy cho đến khi không còn nút con nào
            self.preorder(
                troot._right
            )  # cho đến khi không còn nút con nào, trước khi tiếp tục duyệt nút con bên phải
    ...
    def postorder(
        self, troot
    ):  # hàm duyệt cây theo postoder (duyệt từng nút con bên trái của một nút theo thứ tự trước và thực hiện đệ quy cho đến khi không còn nút con nào, trước khi tiếp tục duyệt nút con bên phải.)
        if troot:  # kiểm tra nếu troot không phải là None
            self.postorder(
                troot._left
            )  # duyệt từng nút con bên trái của một nút theo thứ tự trước và thực hiện đệ quy cho đến khi không còn nút con nào
    ...
    def levelorder(self):  # duyệt theo mức level
        ...
        while not Q.isempty():  # chừng nào Q còn khác rỗng thì
            ...
            if t._left:  # nếu t có nút con bên trái thì
                ...
                Q.enqueue(
                    t._left
                )  # gán giá trị nút con bên trái vào cuối hàng đợi đ tiếp tục duyệt ở vòng lặp sau
            if t._right:  # nếu t có nút con bên phải thì
                print(t._right._element, end=" ")  # in ra giá trị nút con bên phải
                Q.enqueue(
                    t._right
                )  # gán giá trị nút con bên phải vào cuối hàng đợi đ tiếp tục duyệt ở vòng lặp sau
    ...
    def count(self, troot):  # hàm đếm số lượng node của cây
        if troot:  # nếu nút hiện tại có giá trị
            ...
            return (
                x + y + 1
            )  # đệ quy  để tính số lượng node của cây theo công thức x+y+1
        return 0
    ...
    def height(self, troot):  # tính chiều cao của cây
        if troot:  # nếu nút hiện tại không phải là None
            x = self.height(
                troot._left
            )  # Đệ quy để tính chiều cao của cây con bên trái
            y = self.height(
                troot._right
            )  # Đệ quy để tính chiều cao của cây con bên phải
            if x > y:
                return (
                    x + 1
                )  # Nếu chiều cao của cây con bên trái lớn hơn, trả về chiều cao của nó cộng thêm 1
            else:
