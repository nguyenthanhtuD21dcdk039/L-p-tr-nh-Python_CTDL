...
class StacksLinked:
    ...
    def push(self, e):
        newest = _Node(
            e, None
        )  # Tạo một nút mới với giá trị là e và con trỏ next là None
        if self.isempty():
            self._top = newest  # Nếu ngăn xếp trống, nút mới là đỉnh ngăn xếp
        else:
            newest._next = (
                self._top
            )  # Ngược lại, con trỏ next của nút mới trỏ đến nút hiện tại là đỉnh ngăn xếp
    ...
    def pop(self):
        if self.isempty():
            print(
                "Stack is Empty"
            )  # Nếu ngăn xếp trống, in ra thông báo và thoát khỏi phương thức
            return
