...
class BinarySearchTree:  # Hàm khởi tạo của lớp BinarySearchTree, tạo một cây mới với gốc là None
    def __init__(
        self,
    ):  # Phương thức chèn một phần tử e vào cây tìm kiếm nhị phân không đệ quy
        self._root = None
    ...
    def insert(self, troot, e):
        ...
        while troot:  # kiểm tra nếu troot đang xét tới không rỗng
            ...
            if (
                e == troot._element
            ):  # kiểm tra nếu giá trị e cần chèn vào bằng giá trị troot hiện tại thì return
                return
            elif (
                e < troot._element
            ):  # nếu e nhỏ hơn giá trị troot thì xét tới con bên trái của troot
                troot = troot._left
            elif (
                e > troot._element
            ):  # nếu e lớn hơn giá trị troot thì xét tới con bên phải của troot
                troot = troot._right
        n = _Node(e)  # tạo một nút n mới để chèn vào cây
        if self._root:  # kiểm tra nêu cây không rỗng thì
            if (
                e < temp._element
            ):  # nếu giá trị e cần chèn nhỏ hơn giá trị của temp đang xét trước đó thì chèn vào bên trái vị trí temp
                temp._left = n
    ...
    def rsearch(self, troot, key):  # hàm search có đệ quy
        if troot:  # Kiểm tra nếu nút hiện tại tồn tại (khác None)
            if (
                key == troot._element
            ):  # Nếu giá trị cần tìm bằng giá trị của nút hiện tại
                return True  # Trả về True vì đã tìm thấy giá trị trong cây
            elif (
                key < troot._element
            ):  # Nếu giá trị cần tìm nhỏ hơn giá trị của nút hiện tại
                return self.rsearch(
                    troot._left, key
                )  # Gọi đệ quy với nút con bên trái của nút hiện tại
            elif (
                key > troot._element
            ):  # Nếu giá trị cần tìm lớn hơn giá trị của nút hiện tại
                return self.rsearch(
                    troot._right, key
                )  # Gọi đệ quy với nút con bên phải của nút hiện tại
        else:
    ...
    def delete(self, e):  # hàm xóa
        ...
        while (
            p and p._element != e
        ):  # Duyệt cây cho đến khi tìm thấy nút có giá trị bằng e hoặc p trở thành None
            pp = p  # Lưu trữ nút cha của nút p
            if e < p._element:
                p = (
                    p._left
                )  # Di chuyển xuống nút con bên trái nếu giá trị cần xóa nhỏ hơn giá trị của nút p
            else:
                p = (
                    p._right
                )  # Di chuyển xuống nút con bên phải nếu giá trị cần xóa lớn hơn giá trị của nút p
        if not p:
        ...
        if p._left and p._right:  # kiểm tra nếu nút cần xóa có cả hai con
            ...
            while (
                s._right
            ):  # Tìm nút thay thế s với giá trị lớn nhất trong cây con bên trái của p
                ps = s  # Khởi tạo con trỏ ps là cha của s. Ban đầu, ps được gán bằng p.
                s = s._right  # di chuyển s sang cây con bên phải
            p._element = (
                s._element
            )  # Thay đổi giá trị của nút p thành giá trị của nút thay thế s
