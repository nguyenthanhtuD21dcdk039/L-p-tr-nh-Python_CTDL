class _Node:  # Định nghĩa lớp _Node để tạo nút cho danh sách liên kết vòng
    __slots__ = (
        "_element",
        "_next",
    )  # Mỗi nút chứa hai thuộc tính là _element (giá trị của nút) và _next (tham chiếu đến nút tiếp theo)

    def __init__(
        self, element, next
    ):  # Phương thức khởi tạo của lớp _Node, được gọi khi một nút mới được tạo
        self._element = (
            element  # Gán giá trị của tham số element cho thuộc tính _element của nút
        )
...
class CircularLinkedList:  # Định nghĩa lớp CircularLinkedList để tạo danh sách liên kết vòng
    def __init__(self):  # Phương thức khởi tạo của lớp CircularLinkedList
        ...
        self._tail = (
            None  # Tạo một biến tail để tham chiếu đến nút cuối cùng (ban đầu là None)
        )
        self._size = (
            0  # Tạo một biến _size để đếm số phần tử của danh sách liên kết vòng
        )

    def __len__(
        self,
    ):  # Định nghĩa phương thức __len__ để kiểm tra số phần tử của danh sách liên kết
        return self._size  # Trả về độ dài (số phần tử) của danh sách liên kết

    def isempty(
        self,
    ):  # Định nghĩa phương thức isempty để kiểm tra xem danh sách liên kết có rỗng không
        return (
            self._size == 0
        )  # Trả về giá trị True nếu danh sách rỗng (size = 0), False nếu danh sách có phần tử

    def addlast(
        self, e
    ):  # Định nghĩa phương thức addlast để thêm phần tử e vào cuối danh sách liên kết vòng
        newest = _Node(
            e, None
        )  # Tạo một nút mới (newest) với giá trị e và tham chiếu _next ban đầu là None
        if self.isempty():  # Nếu danh sách đang rỗng
            ...
            newest._next = (
                self._tail._next
            )  # Tham chiếu _next của newest trỏ vào nút đầu tiên hiện tại (vì self._tail._next trỏ vào đầu danh sách)
    ...
    def addfirst(
        self, e
    ):  # Định nghĩa phương thức addfirst để thêm phần tử e vào đầu danh sách liên kết vòng
        newest = _Node(
            e, None
        )  # Tạo một nút mới (newest) với giá trị e và tham chiếu _next ban đầu là None
        if self.isempty():  # Nếu danh sách rỗng
            ...
            newest._next = (
                self._head
            )  # Tham chiếu _next của nút newest trỏ vào nút đầu tiên hiện tại của danh sách, tạo newest thành nút đầu tiên của danh sách
    ...
    def addany(
        self, e, position
    ):  # Định nghĩa phương thức addany để thêm phần tử e vào vị trí position
        newest = _Node(
            e, None
        )  # Tạo một nút mới (newest) với giá trị e và tham chiếu _next ban đầu là None
        ...
        while (
            i < position - 1
        ):  # Duyệt qua danh sách để đến phần tử cần chèn (position - 1)
            p = p._next  # Duyệt tới nút tiếp theo
            i = i + 1
        newest._next = (
            p._next
        )  # Tham chiếu _next của nút newest trỏ vào tham chiếu của nút kế tiếp của p
    ...
    def removefirst(
        self,
    ):  # Định nghĩa phương thức removefirst để xóa phần tử đầu tiên của danh sách liên kết vòng
        if self.isempty():  # Nếu danh sách rỗng
        ...
        self._tail_next = (
            self._head._next
        )  # Cập nhật tham chiếu của phần tử cuối cùng, là tham chiếu đến phần tử kế tiếp của phần tử đầu tiên
    ...
    def display(
        self,
    ):  # Định nghĩa phương thức display để hiển thị phần tử của danh sách liên kết vòng
        p = self._head  # Tham chiếu p trỏ vào nút đầu tiên của danh sách liên kết
        i = 0  # Gán i = 0
