class _Node:  # Định nghĩa lớp _Node (nút)
    __slots__ = (
        "_element",
        "_next",
    )  # Xác định thuộc tính của lớp _Node: _element và _next
...
class CircularLinkedList:  # Định nghĩa lớp CircularLinkedList (danh sách liên kết vòng)
    ...
    def isempty(
        self,
    ):  # Định nghĩa phương thức kiểm tra xem danh sách liên kết có rỗng không
        return (
            self._size == 0
        )  # Trả về giá trị True nếu danh sách rỗng (size = 0), ngược lại trả về False

    def addlast(
        self, e
    ):  # Định nghĩa phương thức thêm phần tử e vào cuối danh sách liên kết vòng
        newest = _Node(
            e, None
        )  # Tạo một nút mới có giá trị e và tham chiếu _next ban đầu là None
        if self.isempty():  # Kiểm tra nếu danh sách liên kết đang rỗng
            ...
            newest._next = (
                self._tail._next
            )  # Nút mới trỏ vào nút đầu tiên hiện tại của danh sách liên kết
    ...
    def addfirst(
        self, e
    ):  # Định nghĩa phương thức thêm phần tử e vào phần tử đầu của danh sách liên kết
        newest = _Node(
            e, None
        )  # Tạo một nút mới có giá trị e và tham chiếu _next ban đầu là None
        if self.isempty():  # Kiểm tra nếu danh sách rỗng
            ...
            self._tail._next = (
                newest  # Tham chiếu _next của nút cuối cùng hiện tại trỏ vào nút mới
            )
            newest._next = (
                self._head
            )  # Tham chiếu _next của nút mới trỏ vào nút đầu tiên hiện tại của danh sách, tạo nút mới thành nút thứ hai của danh sách
            self._head = newest  # Cập nhật tham chiếu _head để trỏ vào nút mới, nút mới trở thành phần tử đầu tiên của danh sách
        self._size += 1  # Tăng kích thước danh sách liên kết lên 1

    def addany(
        self, e, position
    ):  # Định nghĩa phương thức thêm phần tử e vào vị trí position trong danh sách liên kết
        newest = _Node(
            e, None
        )  # Tạo một nút mới có giá trị e và tham chiếu _next ban đầu là None
        ...
        newest._next = (
            p._next
        )  # Tham chiếu của nút mới trỏ vào tham chiếu của nút kế tiếp của p
        p._next = newest  # Tham chiếu của nút p trỏ vào nút mới, tạo thành liên kết (p --> newest --> p.next)
        self._size += 1  # Tăng kích thước danh sách liên kết lên 1

    def removefirst(
        self,
    ):  # Định nghĩa phương thức xóa phần tử đầu tiên khỏi danh sách liên kết vòng
        if self.isempty():  # Kiểm tra nếu danh sách rỗng
        ...
        self._tail_next = (
            self._head._next
        )  # Cập nhật tham chiếu của phần tử cuối cùng để trỏ vào phần tử kế tiếp của phần tử đầu tiên, tương đương việc bỏ qua phần tử đầu tiên
        self._head = (
            self._head._next
        )  # Cập nhật _head để trỏ vào phần tử kế tiếp, loại bỏ phần tử đầu tiên
    ...
    def removelast(
        self,
    ):  # Định nghĩa phương thức xóa phần tử cuối cùng khỏi danh sách liên kết vòng
        if self.isempty():  # Kiểm tra nếu danh sách rỗng
            print("Circular List is Empty")  # In ra thông báo rằng danh sách vòng rỗng
            return
        p = self._head  # Gán p để trỏ vào phần tử đầu tiên của danh sách liên kết
        ...
        p._next = (
            self._head
        )  # Cập nhật tham chiếu của phần tử gần cuối là phần tử đầu tiên của danh sách liên kết, tương tự việc bỏ qua phần tử cuối cùng
        self._size -= 1  # Giảm kích thước danh sách đi 1
    ...
    def display(
        self,
    ):  # Định nghĩa phương thức để hiển thị các phần tử trong danh sách liên kết vòng
        p = self._head  # Gán p để trỏ vào phần tử đầu tiên của danh sách liên kết
        i = 0  # Khởi tạo biến đếm i
        while i < len(self):  # Sử dụng vòng lặp để chạy đến phần tử cuối cùng
            print(
                p._element, end="-->"
            )  # In ra giá trị của p, kết thúc bằng dấu mũi tên
            p = p._next  # Di chuyển p để tham chiếu tới phần tử kế tiếp
            i = i + 1  # Tăng biến đếm i lên 1
        print()  # In xuống dòng
