class _Node:  # Tạo một lớp _Node để đại diện cho các nút trong danh sách liên kết đôi
    __slots__ = (
        ...
        "_next",
        "_prev",
    )  # Định nghĩa các thuộc tính của _Node: _element, _next, _prev

    def __init__(
        self, element, next, prev
    ):  # Khởi tạo một đối tượng _Node với các thuộc tính _element, _next, _prev
        self._element = element  # Gán giá trị của tham số 'element' cho thuộc tính '_element' của nút
        self._next = (
            next  # Gán giá trị của tham số 'next' cho thuộc tính '_next' của nút
        )
        self._prev = (
            prev  # Gán giá trị của tham số 'prev' cho thuộc tính '_prev' của nút
        )
...
class DoublyLinkedList:  # Tạo một lớp DoublyLinkedList để đại diện cho danh sách liên kết đôi
    ...
    def __len__(
        self,
    ):  # Phương thức trả về kích thước (số lượng phần tử) của danh sách liên kết đôi
        return self._size
    ...
    def addlast(
        self, e
    ):  # Phương thức thêm phần tử 'e' vào cuối danh sách liên kết đôi
        newest = _Node(
            e, None, None
        )  # Tạo một nút mới chứa giá trị 'e', không trỏ tới nút nào phía trước và sau đó
        if self.isempty():  # Kiểm tra xem danh sách có rỗng không
            ...
            newest._prev = (
                self._tail
            )  # Cập nhật trỏ phía trước của nút mới để trỏ tới nút cuối hiện tại
    ...
    def addfirst(
        self, e
    ):  # Phương thức thêm phần tử 'e' vào đầu danh sách liên kết đôi
        newest = _Node(
            e, None, None
        )  # Tạo một nút mới chứa giá trị 'e', không trỏ tới nút nào phía trước và sau đó
        if self.isempty():  # Kiểm tra xem danh sách có rỗng không
            ...
            newest._next = (
                self._head
            )  # Thay đổi nút kế tiếp của nút mới để trỏ tới nút đầu hiện tại
    ...
    def display(
        self,
    ):  # Phương thức hiển thị danh sách liên kết đôi theo chiều từ đầu tới cuối
        ...
        while p:  # Vòng lặp chạy cho đến khi nút 'p' trỏ tới một nút có giá trị
            print(
                p._element, end="-->"
            )  # In ra giá trị của nút đó, kết thúc bằng dấu mũi tên
    ...
    def addany(self, e, position):  # Phương thức thêm phần tử 'e' vào vị trí 'position'
        ...
        newest._next = (
            p._next
        )  # Thay đổi trỏ tới của nút mới để trỏ tới nút cần chèn, đẩy nút cần chèn lên sau nút mới
        ...
        p._next = (
            newest  # Cập nhật trỏ tới của nút trước nút cần chèn để trỏ tới nút mới
        )
    ...
    def removefirst(self):  # Phương thức xóa phần tử ở vị trí đầu
        ...
        self._head = (
            self._head._next
        )  # Đặt nút thứ hai làm phần tử đầu của danh sách liên kết
    ...
    def removelast(self):  # Phương thức xóa phần tử ở vị trí cuối
        ...
        self._tail = (
            self._tail._prev
        )  # Đặt nút trước cuối làm phần tử cuối của danh sách liên kết
        self._tail._next = (
            None  # Cập nhật trỏ tới của phần tử cuối sau khi xóa để trỏ vào None
        )
        self._size -= 1  # Giảm kích thước của danh sách liên kết
        return e  # Trả về giá trị đã xóa
    ...
    def removeany(self, position):  # Phương thức xóa phần tử tại vị trí 'position'
        ...
        e = (
            p._next._element
        )  # Gán giá trị của phần tử tại vị trí 'position' vào biến 'e'
        p._next = (
            p._next._next
        )  # Cập nhật trỏ tới của vị trí (position - 1) để trỏ tới vị trí (position + 1)
    ...
    def displayrev(
        self,
    ):  # Phương thức hiển thị danh sách liên kết đôi theo chiều từ cuối về đầu
...
print("Size", len(L))  # In ra kích thước của danh sách
L.displayrev()  # Hiển thị danh sách theo chiều từ cuối về đầu
ele = L.removefirst()  # Xóa phần tử ở vị trí đầu
L.display()
print("Size", len(L))  # In ra kích thước của danh sách
L.displayrev()  # Hiển thị danh sách theo chiều từ cuối về đầu
...
print("Size", len(L))  # In ra kích thước của danh sách
L.displayrev()  # Hiển thị danh sách theo chiều từ cuối về đầu
ele = L.removelast()  # Xóa phần tử ở vị trí cuối
L.display()
print("Size", len(L))  # In ra kích thước của danh sách
L.displayrev()  # Hiển thị danh sách theo chiều từ cuối về đầu
print("Removed Element:", ele)
...
print("Size", len(L))  # In ra kích thước của danh sách
L.displayrev()  # Hiển thị danh sách theo chiều từ cuối về đầu
print("Removed Element:", ele)  # In ra giá trị đã xóa
# 048
