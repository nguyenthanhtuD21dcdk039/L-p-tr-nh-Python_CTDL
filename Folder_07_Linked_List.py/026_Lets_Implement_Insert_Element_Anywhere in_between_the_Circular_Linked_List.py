...
class CircularLinkedList:
    ...
    def __init__(self):
        ...
        self._head = (
            None  # Gán tham chiếu đầu danh sách (head) là None (danh sách rỗng)
        )
        self._tail = None  # Khởi tạo biến tail để theo dõi tham chiếu đến nút cuối cùng
        self._size = (
            0  # Khởi tạo biến size để đếm số phần tử của danh sách liên kết vòng
        )
    ...
    def addlast(self, e):
        ...
        newest = _Node(
            e, None
        )  # Tạo một nút mới có giá trị e và tham chiếu ban đầu là None
        ...
        if self.isempty():
            ...
            newest._next = (
                self._tail._next
            )  # Tham chiếu của nút mới trỏ vào nút đầu tiên hiện tại của danh sách
            self._tail._next = (
                newest  # Tham chiếu của nút cuối cùng hiện tại trỏ đến nút mới
            )

        self._tail = (
            newest  # Cập nhật tham chiếu cuối cùng của danh sách để trỏ đến nút mới
        )
    ...
    def addfirst(self, e):
        ...
        self._tail._next = (
            newest  # Cập nhật tham chiếu của nút cuối cùng hiện tại để trỏ đến nút mới
        )
        self._size += 1  # Tăng kích thước của danh sách
