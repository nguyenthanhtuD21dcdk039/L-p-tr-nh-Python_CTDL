...
elelast = (
    L.removelast()
)  # Gọi phương thức removelast() để xóa phần tử cuối cùng và lưu giá trị vào biến elelast
L.display()  # Hiển thị danh sách liên kết sau khi xóa phần tử cuối cùng
print(
    "element remove ", elelast
)  # In ra giá trị của phần tử đã bị xóa từ cuối danh sách liên kết
