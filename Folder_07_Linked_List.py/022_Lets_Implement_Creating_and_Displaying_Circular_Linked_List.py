...
class CircularLinkedList:  # Định nghĩa lớp CircularLinkedList để tạo danh sách liên kết vòng.
    def __init__(self):  # Phương thức khởi tạo của lớp CircularLinkedList.
        ...
        self._size = (
            0  # Khởi tạo biến _size để đếm số phần tử trong danh sách liên kết vòng.
        )
    ...
    def addlast(
        ...
        newest = _Node(
            e, None
        )  # Tạo một nút mới có giá trị là e và tham chiếu next là None.
        if self.isempty():  # Kiểm tra nếu danh sách liên kết đang rỗng.
            ...
            newest._next = (
                self._tail._next
            )  # Nút mới trỏ vào nút đầu tiên hiện tại của danh sách liên kết (vì _tail._next trỏ vào đầu danh sách).
    ...
    def display(
        self,
    ):  # Phương thức để hiển thị các phần tử trong danh sách liên kết vòng.
        p = self._head  # Gán p để trỏ vào nút đầu của danh sách liên kết.
        i = 0  # Khởi tạo biến đếm i.
        while i < len(
            self
        ):  # Sử dụng vòng lặp để duyệt qua danh sách đến phần tử cuối cùng.
            print(
                p._element, end=" --> "
            )  # In giá trị của nút hiện tại, kết thúc bằng dấu ' --> ' để tạo kết nối.
            p = (
                p._next
            )  # Di chuyển con trỏ p đến nút tiếp theo trong danh sách liên kết.
            i += 1  # Tăng biến đếm i.
        print()  # In xuống dòng.
